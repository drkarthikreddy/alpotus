...
(function() {
    function buildData(rawText) {
        ...
        const formatBlock = (val, type) => {
            ...
            if (type === 'TABLE') {
                const tables = val.split(/\n(?=TABLE:)/); // Split if there are multiple table labels
                return tables.map(t => {
                    const rows = t.replace('TABLE:', '').trim().split('\n')
                        .map(r => r.trim().replace(/^\||\|$/g, '')) // Remove leading/trailing pipes
                        .filter(r => r.includes('|'));
        ...
        blocks.forEach(block => {
            ...
            // Extract values including multiple occurrences of labels
    ...
    console.log("SubjectData loaded!");
})();
